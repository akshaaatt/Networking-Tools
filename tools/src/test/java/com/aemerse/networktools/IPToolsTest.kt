...
class IPToolsTest {
    ...
    fun testGetLocalAddressReturnsLocalIP() {
        val test: InetAddress = IPTools.localIPv4Address
    ...
    private fun assertIPv4Address(ips: Array<String?>, isIPv4Address: Boolean) {
        for (address in ips) {
            Assert.assertThat(
                ...
                CoreMatchers.`is`<Boolean>(isIPv4Address)
            )
        }
    }
    ...
    private fun assertIPv6Address(ips: Array<String?>, isIPv6Address: Boolean) {
        for (address in ips) {
            Assert.assertThat(
                IPTools.isIPv6Address(address),
                CoreMatchers.`is`<Boolean>(isIPv6Address)
            )
        }
    }
    ...
    private fun assertIPv6StdAddress(ips: Array<String?>, isIPv6StdAddress: Boolean) {
        for (address in ips) {
            Assert.assertThat(
                ...
                CoreMatchers.`is`<Boolean>(isIPv6StdAddress)
            )
        }
    }
}