...
import com.aemerse.networktools.portscanning.PortScanUDP.scanAddress
import com.aemerse.networktools.portscanning.PortScanTCP.scanAddress
import com.aemerse.networktools.PortScan
...
import com.aemerse.networktools.PortScan.PortScanRunnable
import java.lang.InterruptedException
import java.util.Collections
import java.lang.Thread
import kotlin.jvm.Synchronized
import com.aemerse.networktools.portscanning.PortScanUDP
import com.aemerse.networktools.portscanning.PortScanTCP
...
private constructor() {
    ...
    fun setPorts(portString: String?): PortScan {
        ...
        for (x in portString.split(",".toRegex()).toTypedArray()) {
            if (x.contains("-")) {
                val start = x.split("-".toRegex()).toTypedArray()[0].toInt()
                val end = x.split("-".toRegex()).toTypedArray()[1].toInt()
                validatePort(start)
                validatePort(end)
        ...
        override fun run() {
            ...
            when (method) {
                METHOD_UDP -> portScanned(
                    portNo, PortScanUDP.scanAddress(
                        address, portNo, timeOutMillis
                    )
                )
