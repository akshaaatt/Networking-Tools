...
private constructor() {
    ...
    fun setNoPackets(noPackets: Int): WakeOnLan {
        if (noPackets <= 0) throw IllegalArgumentException("Invalid number of packets to send $noPackets")
    ...
    fun setTimeout(timeoutMillis: Int): WakeOnLan {
        if (timeoutMillis <= 0) throw IllegalArgumentException("Timeout cannot be less than zero")
    ...
    fun wake() {
        if (ipStr == null && inetAddress == null) {
            throw IllegalArgumentException("You must declare ip address or supply an inetaddress")
        }
        if (macStr == null) {
    ...
    companion object {
        ...
        fun sendWakeOnLan(ipStr: String?, macStr: String?) {
        ...
        fun sendWakeOnLan(
            ...
            if (ipStr == null) throw IllegalArgumentException("Address cannot be null")
        ...
        fun sendWakeOnLan(
            ...
            if (address == null) throw IllegalArgumentException("Address cannot be null")
            if (macStr == null) throw IllegalArgumentException("MAC Address cannot be null")
            if (port <= 0 || port > 65535) throw IllegalArgumentException("Invalid port $port")
            if (packets <= 0) throw IllegalArgumentException("Invalid number of packets to send $packets")
